...
tooling under the `dev` group of `[dependency-groups]`. The lock file the CI workflow and the
container use is refreshed by CI itself, so a dependency change needs no lock update by hand.
